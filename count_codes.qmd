...
grouped_codes <- all_codes |> 
  ...
  mutate(code_grouping = if_else(nested_level == ">>", cumsum(number), NA_real_)) |> 
  ungroup() |> 
...
Now we can pull out a level 2 code and its children by filtering by a number. For example, 'messaging & calling' is number 86:

```{r}
grouped_codes |> 
  filter(code_grouping == 86) |> 
  pull(code_name)
```
